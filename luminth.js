/* ============================================================
   LUMINTH — Reactive Liquid Staking Frontend (FINAL PRO MAX)
============================================================ */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function connectWallet() {
    ...
    try {
      ...
      await refresh();
initChart(); // AFTER balance is known
  ...
  async function refresh() {
    ...
    const newBalance = Number(ethers.utils.formatUnits(shares, 18));

    animateValue(balanceEl, lastShares, newBalance);
    lastShares = newBalance;
    
    // simulate slow growth if no events yet
if (lastShares > 0) {
  const fakeYield = lastShares * 0.00005; // tiny drift
  updateAPY(fakeYield);
  updateChart(lastShares + fakeYield);
}
  }
  ...
  ============================================================ */
  maxBtn.onclick = () => {
    const value = mode === "stake" ? window.userBalance : window.userShares;
  ...
  pointRadius: 0
    ...
    // seed initial data so chart isn't empty
for (let i = 0; i < 10; i++) {
  chartLabels.push(i);
  chartData.push(lastShares || 0);
}
chart.update();
  ...
  /* ============================================================
     APY
  ============================================================ */
  function updateAPY(yieldAmount) {
    totalYield += yieldAmount;

    const elapsed = (Date.now() - startTime) / 1000;
    if (elapsed < 2 || lastShares === 0) return;

    const apy = (totalYield / lastShares) * (31536000 / elapsed) * 100;

    const el = document.getElementById("apyValue");
    if (isFinite(apy)) el.textContent = apy.toFixed(2) + "%";
  }
  ...
  function listenReactivity() {
    ...
    const v = new ethers.Contract(VAULT_ADDRESS, VAULT_ABI, ws);

    v.on("YieldCompounded", (amount) => {

      const y = Number(ethers.utils.formatUnits(amount, 18));

      status.textContent = `⚡ +${y.toFixed(4)} PONG earned`;

      document.body.classList.add("yield-pulse");
      setTimeout(() => document.body.classList.remove("yield-pulse"), 600);

      showFloatingYield(y);

      updateAPY(y);
      updateChart(lastShares + y);
